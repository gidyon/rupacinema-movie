...
PKG := "github.com/gidyon/rupacinema/movie"
...
docker_build: ## Create a docker image for the service
	docker build --no-cache -t rupacinema-movie:dev .

docker_run: # Run the rupacinema-movie:dev image
	docker run --rm -it -p 11093:11093 -e REDIS_URL=redis rupacinema-movie:dev -uflag

docker_rmi: ## Remove docker image called rupacinema-movie:dev
	docker rmi rupacinema-movie:dev
